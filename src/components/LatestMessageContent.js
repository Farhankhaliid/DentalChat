...
export default class LatestMessageContent extends Component {

  getMessageContent(lastChatItem) {
    const totalLength = 35;
    if (lastChatItem.patient_content && lastChatItem.patient_content.length > 0) {
    ...
    if (lastChatItem.doctor_content && lastChatItem.doctor_content.length > (totalLength - 5)) {
      return 'You : ' + lastChatItem.doctor_content.substring(0, (totalLength - 5)) + '...';
    }
    return 'You : ' + (lastChatItem.doctor_content?lastChatItem.doctor_content:"");
  }
